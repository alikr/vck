...
function Timer(callback, delay, count, immediate) {
...
function interval(callback, delay) {
	...
	checkCall(callback);
	var time = new Timer(callback, delay, count, immediate);
	time.start();
	return time;
}
...
function addInterval(item) {
  if ((typeof item === 'undefined' ? 'undefined' : _typeof(item)) !== 'object' || typeof item.fun !== 'function') return;
  defaultOpt.id++;
  ...
  var _loop = function _loop(_delay) {
    if (!$queue[_delay]['interval']) {
      $queue[_delay]['interval'] = interval(function () {
        $queue[_delay].data.forEach(function (item) {
          item.fun();
        });
      }, +_delay);
    }
...
function removeInterval(key) {
  ...
  var delay = key.slice(0, _pos);
  var id = key.slice(_pos + 1);
  var data = $queue[delay] ? $queue[delay].data : [];
  var index = findIndex(data, function (d) {
    return d.id === +id;
  });
...
var events = [];
function domEvent(type, dom, callback) {
	return window.addEventListener ? dom.addEventListener(type, callback) : dom.attachEvent('on' + type, callback);
}
